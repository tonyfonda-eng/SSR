...
# We now rely exclusively on the robust Canvas sheets.py parser
...
def load_ontology(sheet_url: str) -> None:
    """
    Initializes the semantic dictionary dynamically using parsed headers.
    """
    ...
    try:
        ...
        for c in concepts:
            cid = c.get("Concept_ID", c.get("Concept ID"))
            if not cid: continue
            
            score = float(c.get("Score", c.get("Weight", 1.0)))
            raw_lang = c.get("Languages", "")
            
            phrases = []
            if raw_lang:
                # Parses format: "English: term1, term2; German: term3"
                for lang_block in raw_lang.split(";"):
                    if ":" in lang_block:
                        terms = lang_block.split(":")[1]
                        phrases.extend([t.strip() for t in terms.split(",") if t.strip()])
                    else:
                        phrases.extend([t.strip() for t in lang_block.split(",") if t.strip()])
            
            # Fallback to examples if the languages column was empty
            if not phrases and c.get("Examples"):
                phrases.extend([p.strip() for p in str(c.get("Examples")).split(",") if p.strip()])
                
            if phrases:
                _KNOWLEDGE_GRAPH[cid] = {"phrases": phrases, "weight": score}

        for s in statuses:
            sid = s.get("Status_ID", s.get("Status ID"))
            if not sid: continue
            
            raw_lang = s.get("Languages", "")
            phrases = []
            if raw_lang:
                for lang_block in raw_lang.split(";"):
                    if ":" in lang_block:
